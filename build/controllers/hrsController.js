...
class HrsController {
    list(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const hojasDeRuta = yield database_1.default.query('SELECT * , hr.id as id, suc.nombre as sucursal, CONCAT (ch.nombre, " ",ch.apellido) as chofer, tr.nombre as transporte FROM hojas_de_ruta hr INNER JOIN sucursales suc ON hr.sucursal_id = suc.id INNER JOIN choferes ch ON hr.choferes_id = ch.id INNER JOIN transportes tr ON ch.transporte_id = tr.id');
            res.json(hojasDeRuta);
        });
    }
    getLastHr(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const lastHr = yield database_1.default.query('SELECT id FROM hojas_de_ruta ORDER BY id DESC LIMIT 0, 1');
            res.json(lastHr);
        });
    }
    getOne(req, res) {
...
exports.hrsController = new HrsController;
